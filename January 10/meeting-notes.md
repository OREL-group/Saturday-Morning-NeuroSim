...
## NOTES
AI — underpinnings of CogSci, in its theoretical form.

Biopunk — Organoid Development. #things happing this week. 

Next week: JPM Healthcare (SF).

NeuroTech, Frontier Tower activities.

NIRS, high-density signal (fMRI). Confounds due to blood flow (indirect measure).

utilize GPUs: Jetson 4 (robotics floor). 

utility of GPU —> massively multiparallel. What are other frontier areas using for tech.


GPU: part of IoT — stack of data center.

accomodations for robotics, thinking about mobile power (battery, processor needs to be optimized).

FitBit, Apple Watch. Unsuitable as it needs to be charged daily.

Jetson Thor vs. RPI (for mobile apps). ARM CPU, SOTA GPU.


Fast compute for unstable networks. Diffuse optical tomography for regional, international Hackathons.

more ambitious project goals. Brainhack Toronto.

John Griffiths — Computational Neuroscience, Neurostimulation, TMS/EEG (Focused Ultrasound — unclear where this is going).

Kate — Braingeneers (low-cost protocol). NeuroBioTech (Mind InVitro).


Better Code, Better Science —> what the goals are in academic software —> Neurofedora work.

shell script —> software —> correctly install on a piece of hardware. Lint tools, checks package.

software training —> compiler issues that are low-level. Software engineering is not a science.


AI systems good at low-level maintenance detail. Clear productivity use case. 

Claude Code vs. Jules. Software sustainability project —> companies develop a strategy.

multi-agent architecture communication (agents send each other text messages, roles emerge).

coding agent for Fedora (what is the hole, database for projects).
ITK — development has stalled —> ITK5 needs an update. SBML person, involved in NeuroFedora.

modeling behavior (generative models for behavior).

ActInf — what you want to get at with RL, multiagent RL, multiagent LLMs.


Shaw —> computational Psychiatry.

Yael Niv group: essential software, probabilistic programs.

modeling pathology —> reaction time distribution.

Javascript (mechanical Turk) —> EEG data processing, STAN project (PiMC). Bayesian Uncertainty with Gibbs Sampling.

modeling distributions (Variational Bayes).

Rx infer — new project. Lazy Dynamics. Need for deep probabilistic programming.

network of LLM agents.

React framework —> needs to be functional programming that’s online, active.

interesting multi agent problems.

Interactions — structured complexity. Multiagent + Math Problems.

NeurIPS —> paper for CogSci reading group.

continuous thought machines. Alife, GECCO breakout.

model DNA in compiler way (as opposed to the GoL).

* multiagent LLM, 2024. FutureHouse AI Scientist read, digest multiple papers.


CGS as a network structure (connectionism vs. relationalism).

von Forester — in celebration of Piaget. 

Piagetian terminology is so durable. Adding cybernetic aspect to this.

compositionally, recursiveness. Object tokens.


Dromic definition: racecourse, circular timecourse.

diadrome — pendulum attractors.


Tom Griffiths new paper. LLM agent —> propose generative models.
